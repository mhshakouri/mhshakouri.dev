...
export const resume = {
  ...
  projects: [
    ...
    {
      name: "Arrowword Co-op",
      status: "live, arrowword.mhshakouri.dev",
      highlights: [
        "Public real-time app: several people solve a Persian crossword together on separate devices, from a photo or from an AI-generated puzzle, with push-to-talk voice. Bilingual Persian and English, no accounts, no OCR.",
        "Built AI-first against a written spec: 17 decision records, 18 server-enforced invariants, 368 automated checks. One Durable Object per session serializes every write, so conflict handling costs nothing, and sessions expire themselves through alarms.",
        "Platform limits drove the architecture. Workers allows 10ms of CPU per request, so crossword packing runs in the browser and the server validates the result; voice rides the puzzle's own WebSocket because mainstream calling apps are unreachable on the network it was built for, confirmed by a clip sent from a phone in Iran.",
      ],
      stack: [
        "TypeScript",
        "Cloudflare Workers",
